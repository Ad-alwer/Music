      ...
      <main class="main">
        <form>
          ...
          <div class="d-flex justify-content-evenly mt-4">
            <button class="btn px-3 py-2 signup-btn" @click.prevent="login()">
              login
            </button>
            <button
...
export default {
  ...
  beforeMount() {
    if (Register.methods.getcookies("jwt")) {
      Register.methods.removecookies(150, Register.methods.getcookies("jwt"));
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    checkusername: function (e) {
      ...
      return regexusername.test(e) ? true : false;
    },

    checkpassword: function (e) {
      const passwordregex = /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/;
      return passwordregex.test(e) ? true : false;
    },
    checkmail: function (e) {
      const regexmail = /^[a-zA-Z0-9._%+-]+@gmail\.com$/;
      return regexmail.test(e) ? true : false;
    },

    login: function () {
      const checkpassword = this.checkpassword(
        this.inputsarr.password.inputValue
      );
      const checkusername =
        this.checkusername(this.inputsarr.username.inputValue) ||
        this.checkmail(this.inputsarr.username.inputValue)
          ? true
          : false;
      ...
      if (checkpassword && checkusername) {
        this.loader = true;
        axios
          .post(`${this.apiaddress}users/login`, {
            user: this.$refs.usernameinput.value,
          ...
          .then((res) => {
            if (res.data.status) {
              ...
              Register.methods.setcookies(
                this.$refs.switch.value ? 10 : "session",
                res.data.token
              );
              location.href = "/";
            } else {
